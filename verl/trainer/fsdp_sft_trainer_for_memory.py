...
class FSDPSFTTrainer:
    ...
    def _collate_fn_for_memory_sft(self, batch):
        ...
        if max_memory_num == 0 and max_memory_len == 0:
            ...
            batched_memory_input_ids = torch.full((len(batch), max_memory_num, max_memory_len), self.tokenizer.pad_token_id)
        ...
        return {
            "input_ids": batched_input_ids.to(torch.long),
            "attention_mask": batched_attention_mask.to(torch.long),
            "loss_mask": batched_loss_mask.to(torch.long),
            "position_ids": batched_position_ids.to(torch.long),
            "memory_input_ids": batched_memory_input_ids.to(torch.long),
            "memory_attention_mask": batched_memory_attention_mask.to(torch.long),
        }
...
def run_sft(config):
    ...
    # build processor and datasets first
    ...
    trainer.train_sampler.set_epoch(0)
...
def create_sft_dataset(data_paths, data_config, processor):
    """Create the MemorySFT dataset."""

    return MemorySFTDataset(parquet_files=data_paths, processor=processor, config=data_config)
