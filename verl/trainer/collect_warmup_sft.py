...
class MemoryLLMInputBuilder:
    ...
    def _make_memory_part(self, memory_content: str) -> str:
        return {'type': 'memory_text', 'memory_text': {'text': memory_content}}
    ...
    def __call__(self, obs: str, episodes: List["Episode"], **kwargs) -> str:
        ...
        for episode in episodes:
            if episode.status == "finished":
                messages.append({
                    "content": [self._make_memory_part(episode.current_observation)],
                    "role": "user",
                })
                messages.append({
...
class MemoryLLMInputBuilderNoEmbedding:
    def __init__(self, system_prompt: str):
        self.system_prompt = system_prompt
    ...
    def __call__(self, obs: str, episodes: List["Episode"], **kwargs) -> str:
        ...
        for episode in episodes:
            if episode.status == "finished":
                messages.append({
                    "content": episode.current_observation,
                    "role": "user",
                })
                messages.append({
                    "content": episode.llm_response,
...
def main(config):
    ...
    if config.env.env_name == "alfworld/AlfredTWEnv":
        if "no_embedding" in config.trainer.experiment_name:
            print("Using No embedding, make everything text")
            input_builder = MemoryLLMInputBuilderNoEmbedding(system_prompt=alfworld_system_prompt)
        else:
    ...
    for bi, batch_dict in tqdm(enumerate(val_loader), total=len(val_loader), desc="Validation"):
        ...
        all_trajs.extend(prompt_batch.trajs)

    import pickle
    save_path = f"{config.trainer.experiment_name}_all_trajs_{config.env.env_name}.pkl"
